...
async function verifyDefectsAdded() {
  try {
    ...
    // Show all defects with their relationships
    console.log('\n🐛 ALL DEFECTS WITH RELATIONSHIPS:');
    const [allDefects] = await sequelize.query(`
      ...
      LEFT JOIN priority pr ON d.priority_id = pr.id
      ORDER BY d.id
    `);

    allDefects.forEach((d, index) => {
      console.log(`\n${index + 1}. ${d.defect_id}: "${d.description}"`);
      console.log(`   Assigned: ${d.assigned_by} → ${d.assigned_to}`);
      console.log(`   Project: ${d.project_name} | Module: ${d.module_name}`);
      console.log(`   Type: ${d.defect_type_name} | Status: ${d.defect_status_name}`);
      console.log(`   Priority: ${d.priority_name} | Severity: ${d.severity_name}`);
    });
